...
actor {
  ...
  stable var _autoSeed : () = ();
  stable var productsSeeded : Bool = false;

  stable var stableProducts : [Product] = [];
  stable var stableOrders : [Order] = [];
  stable var stableUserProfiles : [(Principal, UserProfile)] = [];
  stable var nextProductId : Nat = 29;
  stable var nextOrderId : Nat = 1;
  stable var deliverySettings : DeliverySettings = { forwardRate = 10.0; reverseRate = 4.0 };
  ...
  // Called from frontend once to seed default products
  public shared func seedDefaultProducts() : async () {
    ...
    for (p in defaults.vals()) { products.add(p.id, p) };
    stableProducts := products.values().toArray();
    productsSeeded := true;
  };
  ...
  public shared func addProduct(
    name : Text, category : Text, price : Float, unit : Text, imageUrl : Text
  ) : async Nat {
    let id = nextProductId;
    products.add(id, { id; name; category; price; unit; imageUrl; available = true });
    stableProducts := products.values().toArray();
    nextProductId += 1;
    id;
  };
  ...
  public shared func updateProduct(
    id : Nat, name : Text, category : Text, price : Float,
    unit : Text, imageUrl : Text, available : Bool
  ) : async Bool {
  ...
  public shared func placeOrder(
    customerId : Text, customerName : Text, customerPhone : Text, village : Text,
    items : [OrderItem], subtotal : Float, distanceKm : Float,
    deliveryCharge : Float, total : Float, latitude : Float, longitude : Float
  ) : async Nat {
    let id = nextOrderId;
    orders.add(id, {
      id; customerId; customerName; customerPhone; village; items;
      subtotal; distanceKm; deliveryCharge; total;
      status = "Pending"; latitude; longitude; timestamp = Time.now();
    });
    stableOrders := orders.values().toArray();
    nextOrderId += 1;
    id;
  ...
  public query func getOrdersByCustomer(customerPhone : Text) : async [Order] {
    orders.values().toArray().filter(func(o : Order) : Bool { o.customerPhone == customerPhone });
  };
  ...
  public shared func markDelivered(orderId : Nat) : async Bool {
    switch (orders.get(orderId)) {
      case (?order) {
        orders.add(orderId, {
          id = order.id; customerId = order.customerId;
          customerName = order.customerName; customerPhone = order.customerPhone;
          village = order.village; items = order.items;
          subtotal = order.subtotal; distanceKm = order.distanceKm;
          deliveryCharge = order.deliveryCharge; total = order.total;
          status = "Delivered"; latitude = order.latitude;
          longitude = order.longitude; timestamp = order.timestamp;
        });
        stableOrders := orders.values().toArray();
        true;
      };
